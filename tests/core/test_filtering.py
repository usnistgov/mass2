...
def ATSF(pulse, npre, noise, sample_time_sec, peak=0.0, f_3db=None, cut_pre=0, cut_post=0):
    ...
    maker = mass2.core.FilterMaker(pulse[:, 0], npre, noise, dt_model=pulse[:, 1], sample_time_sec=sample_time_sec, peak=peak)
...
def test_dc_insensitive():
    ...
    maker_no_psd = mass2.core.FilterMaker(
...
def test_no_concrete_baseFilter():
    ...
    with pytest.raises(TypeError):
        _ = mass2.core.Filter(np.zeros(100), 100.0, 100.0, 100.0)
...
class TestWhitener:
    ...
    def test_trivial():
        ...
        w = mass2.core.ToeplitzWhitener([1.0], [1.0])  # the trivial whitener
    ...
    def test_reversible():
        ...
        w = mass2.core.ToeplitzWhitener([1.0, -1.7, 0.72], [1.0, 0.95])
        r = np.random.default_rng().standard_normal(100)
