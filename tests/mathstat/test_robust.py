...
class Test_Shorth:
    ...
    def test_unnormalized():
        ...
        assert r == x[3] - x[1], "Did not find shortest half range in length-5 list"
        assert shr_mean == x[1:4].mean(), "Did not find shortest half mean in length-5 list"
        assert shr_ctr == 0.5 * (x[1] + x[3]), "Did not find shortest half center in length-5 list"
        ...
        assert shr_mean == x[1:5].mean(), "Did not find shortest half mean in length-6 list"
        assert shr_ctr == 0.5 * (x[1] + x[4]), "Did not find shortest half center in length-6 list"
    ...
    def test_sort_inplace():
        ...
        assert y[0] == 1, "shorth_range has not sorted a ndarray in place when requested to do so."
...
class Test_High_Median:
    ...
    def test_weighted():
        ...
        assert high_median(x, w) == 3, "Failed high_median on balanced, odd-summed weights."
        x, w = scramble(x, w)
        assert high_median(x, w) == 3, "Failed high_median on balanced, odd-summed weights."

        x, w = [1, 2, 3, 4, 5], [3, 1, 2, 1, 3]
        assert high_median(x, w) == 3, "Failed high_median on balanced, even-summed weights."
        x, w = scramble(x, w)
        assert high_median(x, w) == 3, "Failed high_median on balanced, even-summed weights."

        x, w = [1, 2, 3, 4, 5], [3, 1, 1, 1, 1]
        assert high_median(x, w) == 2, "Failed high_median on unbalanced odd-summed weights."
        x, w = scramble(x, w)
        assert high_median(x, w) == 2, "Failed high_median on unbalanced odd-summed weights."

        x, w = [1, 2, 3, 4, 5], [4, 1, 1, 1, 1]
        assert high_median(x, w) == 2, "Failed high_median on even-summed weights."
        x, w = scramble(x, w)
        assert high_median(x, w) == 2, "Failed high_median on even-summed weights."
...
class Test_Qscale:
    ...
    def test_simple():
        ...
        assert Qscale(x) == pytest.approx(2.0 * 2.2219 * 0.844, abs=1e-3)
