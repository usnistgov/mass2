...
class Test_LaplaceEntropy:
    ...
    def test_specific_values_entropy():
        ...
        e = laplace_entropy([1, 2, 3, 0.1, 0.5, 0.3, 0.9, -5, 3], 0.2)
        assert e == pytest.approx(1.4343995252696242)
        e = laplace_entropy([1, 2, 3, 0.1, 0.5, 0.3, 0.9, -5, 3], 1.0)
        assert e == pytest.approx(2.2473133161879617)
        e = laplace_entropy([1, 2, 3, 0.1, 0.5, 0.3, 0.9, -5, 3], 5.0)
    ...
    def test_specific_values_cross_h():
        ...
        e = laplace_cross_entropy(np.linspace(1, 3, 30), [1, 2, 3.0], 0.2)
        assert e == pytest.approx(1.39061512214)
        e = laplace_cross_entropy(np.linspace(1, 3, 30), [1, 2, 3.0], 1)
        assert e == pytest.approx(2.19810978807)
        e = laplace_cross_entropy(np.linspace(1, 3, 30), [1, 2, 3.0], 5)
        assert e == pytest.approx(3.77065709074)
        e = laplace_cross_entropy(np.linspace(1, 3, 30), np.linspace(1, 3, 30), 0.2)
    ...
    def test_exact_approx_entropy():
        ...
        z = np.hstack([x - 0.001, x - 0.0005, x, x + 0.0002, x + 0.0008])
        ...
        e = laplace_entropy(z, 0.1, "exact")
        assert e == pytest.approx(0.8215581872670996)
        e = laplace_entropy(z, 0.1)
        assert e == pytest.approx(0.8215581872670996)
        e = laplace_entropy(z, 0.1, "approx")
    ...
    def test_exact_approx_cross_entropy():
        ...
        x = np.linspace(-1, 1, 1001)
        z = np.hstack([x - 0.001, x - 0.0005, x - 0.0001, x + 0.0002, x + 0.0008])
        # Because these are size 5005 vectors, they should default to "exact" mode.
        ...
        e = laplace_cross_entropy(z, x, 0.1, "exact")
        assert e == pytest.approx(0.8366756130721216)
        e = laplace_cross_entropy(z, x, 0.1)
        assert e == pytest.approx(0.8366756130721216)
        e = laplace_cross_entropy(x, z, 0.1, "exact")
        assert e == pytest.approx(0.8321207511380377)
        e = laplace_cross_entropy(x, z, 0.1)
        assert e == pytest.approx(0.8321207511380377)
        e = laplace_cross_entropy(z, x, 0.1, "approx")
        assert e == pytest.approx(0.8189687816571245)
        e = laplace_cross_entropy(x, z, 0.1, "approx")
    ...
    def test_types():
        ...
        for t2 in (int, float, np.float32):
            ...
            for t1 in (float, np.float32):
                e = laplace_cross_entropy(np.linspace(1, 3, 30, dtype=t1), np.array([1, 2, 3], dtype=t2), 0.2)
                assert e == pytest.approx(1.39061512214)
                e = laplace_KL_divergence(np.linspace(1, 3, 30, dtype=t1), np.array([1, 2, 3], dtype=t2), 0.2)
    ...
    def test_bug115():
        ...
        x = mass2.spectra["MnKAlpha"].rvs(size=100, rng=rng, instrument_gaussian_fwhm=0)
    ...
    def test_bug116():
        ...
        rng = np.random.default_rng(100)
        x = mass2.spectra["MnKAlpha"].rvs(size=1000, rng=rng, instrument_gaussian_fwhm=0)
