...
class Test_fit_kink:
    ...
    def test_noisless_fit(self):
        ...
        _, (kbest, a, b, c), X2 = mass.mathstat.fitting.fit_kink_model(self.x, self.y, kbounds=(3, 6))
        assert X2 < 1e-8
    ...
    def test_noisless_fit_no_bounds(self):
        ...
        _, (kbest, a, b, c), X2 = mass.mathstat.fitting.fit_kink_model(self.x, self.y, kbounds=None)
        assert X2 < 1e-8
        assert abs(kbest - self.truek) < 1e-5
        assert abs(a - self.truek) < 1e-5
        assert abs(b - 1) < 1e-5
    ...
    def test_noisy_fit(self):
        ...
        _, (kbest, a, b, c), X2 = mass.mathstat.fitting.fit_kink_model(self.x, noisy_y, kbounds=(3, 6))
        assert X2 < 1.0
