...
class Test_MnKA_distribution:
    ...
    def test_location_against_STANDARD_FEATUES():
        for name, spectrum in mass2.spectra.items():
            ...
            if not result:
                msg = f"{name} spectrum.nominal_peak_energy={spectrum.nominal_peak_energy}, mass2.STANDARD_FEATURES={target}"
...
class TestAddFitter:
    ...
    def test_add_same_line_fails():
        ...
        mass2.spectra["dummyKBeta"].model()
        mass2.spectra["dummyKBeta"].model(has_tails=True)
        mass2.spectra["dummyKBeta"].fitter()
        mass2.spectra["dummyKBeta"].model()
    ...
    def test_some_lines_make_sense():
        assert mass2.spectra["MnKAlpha"].nominal_peak_energy == 5898.802
        assert mass2.spectra["AuLAlpha"].nominal_peak_energy == 9713.44
