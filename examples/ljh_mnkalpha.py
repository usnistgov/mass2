...
def _(moss, pulsedata):
    ...
    data = moss.Channels.from_ljh_folder(pulse_folder=_p.pulse_folder, noise_folder=_p.noise_folder)
    data
    return (data,)
...
def _(data, mo, moss, plt):
    ...
    pfresult = moss.rough_cal.peakfind_local_maxima_of_smoothed_hist(uncalibrated, fwhm_pulse_height_units=fwhm_pulse_height_units)
    pfresult.plot()
...
def _(data2, mo):
    ...
    _energy_cols
    dropdown_col = mo.ui.dropdown(options=_energy_cols, value=_energy_cols[0], label="energy col")
    mo.md(
...
def _(data2, pl):
    ...
    data3 = data2.map(lambda ch: ch.with_replacement_df(ch.df.select(pl.exclude("pulse")).sort(by="timestamp")))
...
def _(data3, multifit):
    data4 = data3.map(
        lambda ch: ch.multifit_mass_cal(multifit, previous_cal_step_index=5, calibrated_col="energy2_5lagy_dc"), allow_throw=True
    )
    return (data4,)
...
def _(ch6, mo, np, pl, plt):
    def pfit_dc(line_name, ch):
        ...
        ch2 = ch.with_columns(
            ch.df.select(pl.col("energy_5lagy") - slope * (pl.col("pretrig_mean") - ptm)).rename({
                "energy_5lagy": f"energy_5lagy_dc_{line_name}"
            })
        )
        return ch2
