...
# Define the dtype for the extended structured array returned by summarize_data_numba_extended.
# This is opt-in: it does not affect summarize_data_numba or result_dtype above.
extended_result_dtype = np.dtype([
...
@njit
def summarize_data_numba_extended(  # noqa: PLR0914, PLR0917
    rawdata: NDArray[np.uint16],
    timebase: float,
    peak_samplenumber: int,
    pretrigger_ignore_samples: int,
    ...
    """Summarize one segment of the data file, computing an extended set of summary statistics.

    This is an opt-in alternative to summarize_data_numba: it returns everything
    summarize_data_numba does, plus additional fields (see extended_result_dtype),
    without changing the behavior, output, or performance of summarize_data_numba itself.
    ...
    results = np.zeros(nPulses, dtype=extended_result_dtype)
