...
def _phasecorr_find_alignment(  # noqa: PLR0914
    ...
    use = np.logical_and(np.abs(pulse_heights[:] - peak) < delta_ph, np.abs(phase_indicator) < 2)
    low_phase, median_phase, high_phase = np.percentile(phase_indicator[use], [3, 50, 97])
