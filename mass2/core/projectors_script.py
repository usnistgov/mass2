...
def make_projectors(  # noqa: PLR0917
    pulse_files,
    noise_files,
    h5,
    n_sigma_pt_rms,
    ...
    _ = data.pulse_model_to_hdf5(
        h5,
        n_basis=n_basis,
        maximum_n_pulses=maximum_n_pulses,
        extra_n_basis_5lag=extra_n_basis_5lag,
        noise_weight_basis=noise_weight_basis,
        f_3db_5lag=f_3db_5lag,
        _rethrow=True,
    )
...
def parse_args(fake):
    ...
    parser.add_argument(
        "-r", "--replace_output", help="pass this to overwrite off files with the same path", action="store_true", default=False
    )
    parser.add_argument("-m", "--max_channels", help="stop after processing this many channels", default=2**31, type=int)
    parser.add_argument("--f_3db_ats", help="cutoff frequency for ats filter", default=None, type=float)
    parser.add_argument("--f_3db_5lag", help="cutoff frequency for 5lag filter", default=None, type=float)
    parser.add_argument(
        "--n_ignore_presamples",
...
be able to model the non-flat pretrigger region better"""
    parser.add_argument("--n_sigma_pt_rms", type=float, default=10000, help=help_sigma_pt_rms)
    parser.add_argument(
        "--n_sigma_max_deriv", type=float, default=8, help="passed to autocuts to determine pulses used to generate pulse model"
    )
    parser.add_argument("-n", "--n_basis", type=int, default=5, help="how many projectors to generate, must be >=3")
    parser.add_argument(
        "--maximum_n_pulses", type=int, default=4000, help="maximum number of pulses to take a truncated SVD of, will affect speed"
    )
    ...
    parser.add_argument("-i", "--invert_data", action="store_true", help="pass this flag for downward going pulses (eg RAVEN)")
    parser.add_argument(
        "--dont_optimize_dp_dt", help="simpler derivative like calculating, better for gamma data", action="store_true"
    )
    parser.add_argument(
        "--extra_n_basis_5lag", help="use this many basis components to improve 5 lag filter representation", default=0, type=int
    )
    parser.add_argument(
        "--noise_weight_basis", help="pass false to not apply noise weighting when making projectors", default=False, type=bool
    )
...
def main(args=None):
    ...
    with h5py.File(args.output_path, "w") as h5:
        n_good, n = make_projectors(
            pulse_files=pulse_files,
            noise_files=noise_files,
            h5=h5,
            n_sigma_pt_rms=args.n_sigma_pt_rms,
            n_sigma_max_deriv=args.n_sigma_max_deriv,
            n_basis=args.n_basis,
            maximum_n_pulses=args.maximum_n_pulses,
            mass_hdf5_path=args.mass_hdf5_path,
            mass_hdf5_noise_path=args.mass_hdf5_noise_path,
            invert_data=args.invert_data,
            optimize_dp_dt=not args.dont_optimize_dp_dt,
            extra_n_basis_5lag=args.extra_n_basis_5lag,
            f_3db_ats=args.f_3db_ats,
            f_3db_5lag=args.f_3db_5lag,
