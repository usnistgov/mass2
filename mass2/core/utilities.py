...
# def plot_multipage(
#     data: Channels, subplot_shape: tuple[int, int], helper,
#     filename_template_per_file, filename_template_glob, filename_one_file, format, one_file
# ):
...
def annotate_lines(
    axis: plt.Axes, label_lines: list[str], label_lines_color2: list[str] = [], color1: str = "k", color2: str = "r"
) -> None:
    ...
    for i, label_line in enumerate(label_lines):
        energy = mass2.STANDARD_FEATURES[label_line]
        if yscale == "linear":
