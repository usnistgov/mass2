...
def drift_correct_mass(indicator, uncorrected):
    slope, dc_info = mass.core.analysis_algorithms.drift_correct(indicator, uncorrected)
...
def drift_correct_wip(indicator, uncorrected):
    ...
    return DriftCorrection(
        offset=offset.astype(np.float64), slope=opt_result.x.astype(np.float64)
    )
...
class DriftCorrectStep(CalStep):
    ...
    def calc_from_df(self, df):
        ...
        df2 = df.select(
            (
                pl.col(uncorrected_col) * (1 + slope * (pl.col(indicator_col) - offset))
            ).alias(self.output[0])
        ).with_columns(df)
        return df2
    ...
    def learn(cls, ch, indicator_col, uncorrected_col, corrected_col, use_expr):
        ...
        indicator_s, uncorrected_s = ch.good_serieses(
            [indicator_col, uncorrected_col], use_expr
        )
