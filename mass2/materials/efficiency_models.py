...
class FilterStack:
    ...
    def add_filter(self, name, material, area_density_g_per_cm2=None, thickness_nm=None,
                   density_g_per_cm3=None, fill_fraction=ufloat(1, 0), absorber=False):
        self.add(Filter.newfilter(name, material, area_density_g_per_cm2=area_density_g_per_cm2,
                                  thickness_nm=thickness_nm, density_g_per_cm3=density_g_per_cm3,
                                  fill_fraction=fill_fraction, absorber=absorber))
    ...
    def get_efficiency(self, xray_energies_eV, uncertain=False):
        ...
        individual_efficiency = np.array([iComponent.get_efficiency(
            xray_energies_eV, uncertain=uncertain) for iComponent in self.components])
...
class Filter:
    ...
    @classmethod
    def newfilter(cls, name, material, area_density_g_per_cm2=None, thickness_nm=None,
                  density_g_per_cm3=None, fill_fraction=ufloat(1, 0), absorber=False):

        material = np.array(material, ndmin=1)
        atomic_number = np.array([xraydb.atomic_number(iMaterial)
                                  for iMaterial in material], ndmin=1)
        ...
        if density_g_per_cm3 is None:
            density_g_per_cm3 = np.array([xraydb.atomic_density(
                int(iAtomicNumber)) for iAtomicNumber in atomic_number], ndmin=1)
        else:
...
def AlFilmWithPolymer(
    ...
    return Filter.newfilter(name=name, material=material, thickness_nm=thickness_nm,
...
def LEX_HT(name):
    ...
    film1 = Filter.newfilter(name='LEX_HT Film', material=film_material,
    ...
    film2 = Filter.newfilter(name='LEX_HT Mesh', material=mesh_material,
...
def get_filter_stacks_dict():
    ...
    EBIT_filter_stack.add_filter(name="Electroplated Au Absorber", material="Au",
                                 thickness_nm=with_fractional_uncertainty(965.5, 0.03), absorber=True)

    EBIT_filter_stack.add(AlFilmWithOxide(
    ...
    filter_50K.add(AlFilmWithOxide(
        name='Al Film', Al_thickness_nm=with_fractional_uncertainty(102.6, 0.02)))
    nickel = Filter.newfilter(name='Ni Mesh', material='Ni', thickness_nm=ufloat(
    ...
    RAVEN1_fs.add_filter(name='Evaporated Bi Absorber', material='Bi',
                         thickness_nm=4.4e3, absorber=True)
    RAVEN1_fs.add(AlFilmWithPolymer(
    ...
    Horton_filter_stack.add_filter(name='Electroplated Au Absorber',
